...
import com.google.maps.gwt.client.Polygon.MouseMoveHandler;
...
public class PolygonMarker extends AbstractShapeMarker implements ShapeMarker {
	...
	private void attachEventHandling() {
		...
		polygon.addMouseMoveListener(new MouseMoveHandler() {
			@Override
			public void handle(MouseEvent event) {
				relayUserAction(UserInteraction.MOUSEMOVE, event.getLatLng());
				logger.finer("Mouse move polygon "+getFillColour());
			}
		});
		...
		polygon.addMouseOverListener(new MouseOverHandler() {
			...
			public void handle(MouseEvent event) {
				...
				logger.finer("Mouse over polygon "+getFillColour());
			}
		});
		polygon.addMouseOutListener(new MouseOutHandler() {
			...
			public void handle(MouseEvent event) {
				...
				logger.finer("Mouse out polygon "+getFillColour());
			}
		});
	}
